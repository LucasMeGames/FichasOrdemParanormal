...
if (isset($_SESSION["UserID"])) {
    ...
    if (!empty($_POST["nome"])) {
        ...
        if (!preg_match('/^[a-zA-Z áéíóúãõàèìòùÁÉÍÓÚÃÕÀÈÌÒÙ]*$/', $nome)) {
    ...
    if ($san == 1) {
        $san = calcularsan($nex, $classe);
	    if($origem == 7){
		    $san = 10 + (5 * (floor(($nex / 5)) - 1));
	    }
    }                if($san === 1){
		if($rqs["origem"] == 7) {
			switch ($classe){
    ...
    if ($pe == 1) {
        $pe = calcularpe($nex, $classe, $presenca);
    }
    ...
    switch ($origem) {
        ...
        case 1: //Academico
            ...
            $habdes = "Uma vez por cena, quando fizer um teste usando intelecto, você pode gastar 2PE para receber +5 nesse teste.";
        ...
        case 15: // mlitar
            ...
            break;
        case 16: // Operário
            $habnam = "Ferramentas da Profissão (Origem)";
            $habdes = "Escolha uma arma simples ou tática que, a critério do mestre, poderia ser usada como uma ferramenta em sua profissão (como uma marreta para um pedreiro). Você sabe usar a arma escolhida e recebe +1 nas rolagens de dano com ela.";
            $fortitude = $profissao = 5;
            break;
        case 17: // Policiaçl
        ...
        case 20: // Teórico
            ...
            $habdes = "Você não se abala com entidades ou anomalias. Afinal, sempre soube que isso tudo existia. Uma vez por cena, você pode gastar 3 PE para ignorar um dano em Sanidade.";
